...
For Coin Archive's coin list and coin detail pages, should data be fetched by route
...
`ensureQueryData`. Coin Archive has TanStack Query 5.102.8, and the current Start guide
...
Coin Archive's `getRouter()` currently creates no `QueryClient` and exposes no router
...
Coin Archive already sets `defaultPreloadStaleTime: 0`, which is the recommended value
...
The query functions may call Coin Archive's standalone HTTP API from both server and
...
`Route.useLoaderData()` is viable and simpler if Coin Archive chooses Router as its only
...
Coin Archive already depends on and uses TanStack Query, and coin data will
plausibly need cache invalidation after later Coin mutations. Making Router the data
